const nicuSelecto = (niceSelect, realSelect) => {
	...
	realSelect.querySelectorAll('option').forEach((el) => {
		...
		if (!el.getAttribute('disabled') == true) {
			...
			if (el.value) {
				...
				console.log(valTextParis);
			}
			// if (el == realSelect.querySelectorAll('option')[0]) {
			// 	el.classList.add('active')
			// 	realSelect.parentElement.querySelector('p').innerText =
			// 		el.innerText
			// 	realSelect.parentElement
			// 		.querySelector('p')
	...
	lis.forEach((el) => {
		el.addEventListener('click', () => {
			...
			$(`option[value="${filteredOption[0].val}"]`)
				.parent()
				.find('option')
				.removeClass('active')
            ...
            if (realSelect.value != '0') {
                realSelect.parentElement.classList.remove('invalid')
                realSelect.parentElement.classList.remove('invalid-shown')
            } else {
                realSelect.parentElement.classList.remove('invalid')
                realSelect.parentElement.classList.remove('invalid-shown')
			}
